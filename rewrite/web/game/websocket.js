...
export function webSocketOnMessage(event) {
    ...
    if ('p' in alert) {
        // if the alert indicates a reverse promotion is needed then a /move response with 
        // the promotion pick is done, otherwise the normal move communication continues
        if (alert.p === 'rp') {
            // swapActive()
            showPromotion()
        } else if (alert.p === 'rpd') {
            // if a reverse promotion is done then this webpage is waiting for the opponent 
        ...
        // swapActive()
        // with no reverse promotion the normal action is to continue onto this player's next 
        // move choice
        fetchMoves()
    }
}
