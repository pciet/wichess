...
func TestMultipleGoodMatching(t *testing.T) {
	var i int
	c := make(chan struct{})
	m := NewMatcher(1, 2, pairing, func(a string, am interface{}, b string, bm interface{}) {
		i++
		switch a {
		case p1:
			if am != p1r {
				t.Fatalf("%v meta not as expected", p1)
			}
			if b != p2 {
				t.Fatalf("%v matched with %v instead of %v", b, a, p2)
			}
			if bm != p2r {
				t.Fatalf("%v meta not as expected", p2)
			}
		case p2:
			if am != p2r {
				t.Fatalf("%v meta not as expected", p2)
		...
		case p3:
			if am != p3r {
				t.Fatalf("%v meta not as expected", p3)
			}
			if b != p4 {
				t.Fatalf("%v matched with %v instead of %v", b, a, p4)
			}
			if bm != p4r {
				t.Fatalf("%v meta not as expected", p4)
			}
		case p4:
			if am != p4r {
				t.Fatalf("%v meta not as expected", p4)
...
func TestGoodMatching(t *testing.T) {
	c := make(chan struct{})
	m := NewMatcher(1, 3, pairing, func(a string, am interface{}, b string, bm interface{}) {
		if a == p1 {
			if b == p2 {
				c <- struct{}{}
				return
...
func TestMatching(t *testing.T) {
	m := NewMatcher(1, 1, pairing, func(a string, am interface{}, b string, bm interface{}) {
		t.Fatal("paired unexpectedly")
	})
	m.Match(p1, p2r, p1r)
	data := m.Matching(p1)
	if data != p2r {
		t.Fatal("did not get back player metadata from Matching()")
	}
}
